...
const WorkerDashboard: React.FC = () => {
  const [orders, setOrders] = useState<Order[]>(mockOrders.filter(o => o.status === OrderStatus.Pending || o.status === OrderStatus.Packed));
  const [checklists, setChecklists] = useState<{ [orderId: string]: PackingChecklist }>({});
  ...
  const markAsPacked = (orderId: string) => {
    ...
    setOrders(prev => prev.map(o => o.id === orderId ? {...o, status: OrderStatus.Packed} : o));
